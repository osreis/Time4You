/*!
 * Rails 3 Client Side Validations - v3.1.4 FDP!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    'ActionView::Helpers::FormBuilder': {
      add: function(element, settings, message) {
        if (element.data('valid') !== false && jQuery('span.message[for="' + element.attr('id') + '"]')[0] == undefined) {
          var inputErrorField = jQuery(settings.input_tag),
              spanErrorField = jQuery(settings.span_tag),
              span = jQuery('span[for="' + element.attr('id') + '"]:not(.message)');

          if (element.attr('autofocus')) { element.attr('autofocus', false) };
          element.before(inputErrorField);
          inputErrorField.find('span#input_tag').replaceWith(element);
          inputErrorField.find('span.message').attr('for', element.attr('id'));
          spanErrorField.find('span.message').attr('for', element.attr('id'));
          span.replaceWith(spanErrorField);
          spanErrorField.find('span#span_tag').replaceWith(span);
        }
      ...
      remove: function(element, settings) {
        var errorFieldClass = jQuery(settings.input_tag).attr('class'),
            ...
            span = jQuery('span[for="' + element.attr('id') + '"]:not(.message)'),
            spanErrorField = span.closest('.' + errorFieldClass);

        if (inputErrorField[0]) {
          inputErrorField.find('#' + element.attr('id')).detach();
          inputErrorField.replaceWith(element);
          span.detach();
          spanErrorField.replaceWith(label);
        }
      }
    },
